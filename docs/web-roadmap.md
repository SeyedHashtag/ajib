...
| Backups/migrations/restore | CLI operator | All runtimes coordinated, SQLite backup API, safety snapshot | Restore includes extension tables/receipt BLOBs and is exercised | 2/8: shared database CLI implemented; installation-specific recovery evidence remains private |
| Staging/pilot/rollback | Operator, selected pilot users | Opt-in gates, compatible revisions, pending operation preservation | Full journeys pass before expanding access | 8: gated release controls implemented; installation-specific acceptance remains private |
...
- Real-client and production acceptance evidence belongs in private operator
  records; synthetic test results do not attest to a particular installation.
