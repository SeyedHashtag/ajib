...
| Configurations, QR, guidance | Only verified owner can retrieve subscriptions and every direct link; selected QR and private text downloads preserve all links; overlapping reads wait while mutation claims block access; four-language controls and guides remain available | `test_configuration_delivery.py`, `test_security.py`, `test_public_branding.py`, desktop/mobile account journeys |
